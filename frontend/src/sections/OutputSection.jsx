    ...
    <section className="cardSurface">
      ...
      {(status === 'TRANSFORMING' || status === 'REGENERATING') && (
        <div className="cardSurface" style={{ padding: '12px' }}>
          ...
          <div style={{ height: 12, marginTop: 8, background: 'rgba(148,163,184,0.2)', borderRadius: 8 }} />
          <div style={{ height: 12, marginTop: 8, background: 'rgba(148,163,184,0.2)', borderRadius: 8, width: '82%' }} />
        </div>
      )}
      ...
      {status === 'BLOCKED' && (
        <div className="cardSurface" style={{ padding: '12px' }}>
          ...
          <p className="muted">{blockedReason || safetyCopy?.note || 'Try rephrasing with safer wording.'}</p>
        </div>
      )}
