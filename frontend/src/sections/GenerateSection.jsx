...
function GenerateSection() {
  ...
  const setStoreVoicePersonality = useCrashOutStore(
    (state) => state.setVoicePersonality,
  );
      ...
      if (voice === "My Own Voice") {
        ...
        if (!cachedVoiceId) {
          try {
            ...
            const randomPreview =
              designData.previews[
                Math.floor(Math.random() * designData.previews.length)
              ];
            ...
            flashError(
              "Failed to generate custom voice: " + voiceError.message,
            );
            return;
          }
        }
